...
router.get('/callback', (req, res) => {
  ...
  fetch('https://discordapp.com/api/oauth2/token', {
    ...
    .then(json => {
      res.redirect(`/?access_token=${encodeURIComponent(json.access_token)}&refresh_token=${encodeURIComponent(json.refresh_token)}&expires_in=${encodeURIComponent(json.expires_in)}`)
    })
    .catch(err => {
      res.status(400)
      res.send(err)
