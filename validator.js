      ...
      validator(array) {
        array.forEach((edi) => {
          if (!editions.includes(edi))
            throw new Error(
              `Unknown edition "${edi}" in ${JSON.stringify(array)}. Only accepts ${JSON.stringify(
                editions,
              )}`,
            );
        });
        ...
        array.forEach((pair) => {
          ...
          pair[1].forEach((ver) => {
            if (!versions.includes(ver))
              throw new Error(
                `Unknown version "${pair[1]}" in ${JSON.stringify(
                  pair,
                )}. Only accepts ${JSON.stringify(versions)}`,
              );
          });
        });
      },
...
async function single(value, field, parent = undefined) {
  ...
  switch (field.type) {
    case "string":
      if (typeof value !== "string")
        throw new Error(
          `${field.name || JSON.stringify(value)} type is not ${field.type}${parentError}`,
        );
      break;
    case "integer":
    case "float":
  ...
  if ("length" in field && ["string", "array"].includes(field.type)) {
    const extremum =
      typeof field.length === "string"
        ...
        : Object.assign({}, { min: 0 }, field.length);

    if (value.length < extremum.min)
      throw new Error(
        `${field.name || field.type} length lower than ${extremum.min}${parentError}`,
      );
    if ("max" in extremum && value.length > extremum.max)
      throw new Error(
        `${field.name || field.type} length greater than ${extremum.max}${parentError}`,
      );
  }
  ...
  if ("validator" in field) {
    if (typeof field.validator !== "function")
      throw new Error(
        `${field.name} validator is not a function in schema ${JSON.stringify(field)}`,
      );
...
async function validator(obj, schema) {
  if (!Array.isArray(schema))
    throw new Error(`Incorrect schema ${JSON.stringify(schema)} for obj ${JSON.stringify(obj)}`);

  for (let i = 0; i < schema.length; ++i) {
    const field = schema[i];
    // presence
    if (!(field.name in obj) || obj[field.name] === undefined || obj[field.name] === null)
      throw new Error(`Missing field ${field.name} in ${JSON.stringify(obj)}`);
