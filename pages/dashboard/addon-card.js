...
>
  <v-card-text class="pb-0 flex-grow-1 d-flex flex-column">
    ...
    <v-row v-else dense id="status-loader" class="d-flex align-stretch">
      <v-col v-for="i in loading_for" :key="'skeleton-status-' + i" cols="12" :class="['d-flex align-end', loading_for > 1 ? 'col-sm-3' : '']">
        <div class="p mb-0 flex-grow-1 rounded-lg pa-2 d-flex align-center paragraph-loader">
          <div class="d-flex align-end">
            <v-skeleton-loader class="loader mr-1" width="30" height="24" type="heading" /> <v-skeleton-loader class="loader" width="60" min-height="17" type="text" />
          </div>
        </div>
      </v-col>
    </v-row>
    ...
    <v-row class="mt-1 py-0 my-0 align-self-stretch" v-else dense id="stats-loader">
      <v-col v-for="i in 4" :key="'skeleton-stats-' + i" cols="12" sm="3" class="d-flex align-stretch">
        <div class="p mb-0 flex-grow-1 rounded-lg pa-2 d-flex align-center paragraph-loader">
          <div class="d-flex align-end">
            <v-skeleton-loader class="loader mr-1" width="30" height="24" type="heading" /> <v-skeleton-loader class="loader" width="60" min-height="17" type="text" />
          </div>
