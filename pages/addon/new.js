...
const AddonForm = () => import('./addon_form.js')
...
export default {
  ...
  <v-container>
    <div class="text-h4 py-4">
      ...
      <v-progress-circular v-if="names.length == 0" indeterminate/>
    </div>

    <addon-form :names="names"/>
  </v-container>
  `,
  data() {
    return {
  ...
  beforeMount() {
    axios.get('/addons/get/names')
      .then(res => {
        this.names = Object.values(res.data).map(el => el.name)
        this.$forceUpdate()
      })
      .catch(err => {
        console.trace(err)
