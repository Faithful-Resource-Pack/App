...
export default {
  ...
  <v-container>
    ...
    <div class="my-2 text-h5">
      <v-list v-if="Object.keys(addons).length > 0" two-line color="rgba(255, 255, 255, 0.08)" style="background-color: rgba(255,255,255,.05)">
        <v-list-item>
          <v-row>
            <v-col>
  ...
  methods: {
    ...
    isTitleAvailable: function (title) {
      ...
      for (const addonID in this.addons) {
        if (this.addons[addonID].title == title) return false
      }
...
/**
 * TODO: Find a better way to identify if a title is already taken by another add-on
 * con: the current methods needs to loads the entire addons collection and takes several seconds to do so
 * pro: you have all existing title at once
 */