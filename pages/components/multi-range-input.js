...
<v-form lazy-validation ref="form">
  ...
  methods: {
        ...
        list => {
          ...
          while (incorrectValue === undefined && i < list.length) {
            ...
            const correct = function () {
              ...
              return String(Number.parseInt(numbers[0], 10)) === numbers[0] &&
          ...
          return incorrectValue ?
            this.labels.incorrect_value.replace('%value%', incorrectValue) : true
        }
      ]
    },
    isNavCombo: function (keyboard_event) {
      const { key } = keyboard_event
      const is_copy_paste = (keyboard_event.ctrlKey && (key == "c" || key == "x" || key == "v" || key == "a" || key == "u")) || keyboard_event.keyCode == 123;
    ...
    onInput: function (keyboard_event) {
      ...
      if (key !== ' ' && key !== '-' && not_a_num_key
        && !this.isNavCombo(keyboard_event)) {
